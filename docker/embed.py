...
from text_processing import Token, BPEfastApply, SPMApply
        ...
        def batch(tokens, lengths, indices):
            ...
            if not self.left_padding:
                ...
                for i in range(len(tokens)):
                    toks[i, -tokens[i].shape[0]:] = tokens[i]
            return (
                Batch(srcs=None, tokens=toks,
                      lengths=torch.LongTensor(lengths)),
                indices,
            )

        batch_tokens, batch_lengths, batch_indices = [], [], []
        num_tokens = num_sentences = 0
        for i in indices:
            if num_sentences > 0 and (
                    (self.max_tokens is not None and num_tokens + lengths[
                        i] > self.max_tokens)
                    or (
                            self.max_sentences is not None and num_sentences == self.max_sentences)
            ):
...
class LaserTransformerEncoder(TransformerEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if self.sentemb_criterion == "cls":
            ...
            if padding_mask.any():
                x = x.float().masked_fill_(padding_mask, float("-inf")).type_as(
                    x)
...
def encodefilep(
        encoder, inp_file, out_file, buffer_size=10000, fp16=False,
        verbose=False
):
    n = 0
    t = time.time()
...
def encodefile(
    ...
    if not os.path.isfile(out_fname):
        ...
        fin = (
            open(inp_fname, "r", encoding=inp_encoding,
        ...
        encodefilep(
            encoder, fin, fout, buffer_size=buffer_size, fp16=fp16,
            verbose=verbose
        )
        fin.close()
        fout.close()
        ...
        logger.info(
            "encoder: {} exists already".format(os.path.basename(out_fname)))
...
def embed_memory_map(fname, dim=1024, dtype=np.float32, verbose=False):
    nbex = int(os.path.getsize(fname) / dim / np.dtype(dtype).itemsize)
    e = np.memmap(fname, mode="r", dtype=dtype, shape=(nbex, dim))
    if verbose:
        print(
            " - embeddings on disk: {:s} {:d} x {:d}".format(fname, nbex, dim))
    return e
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bpe-codes", type=str, default=None,
        help="Apply BPE using specified codes"
    )
    parser.add_argument(
        "--spm-lang", type=str, default="en",
        help="Apply SPM using specified language"
    )
    parser.add_argument(
        "--spm-model", type=str, default=None,
        help="Apply SPM using specified model"
    )
    ...
    parser.add_argument(
        "--use-hugging-face", action="store_true",
