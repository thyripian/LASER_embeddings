...
base_dir="${LASER}"
tools_ext="${base_dir}/tools-external"
MKDIR "$tools_ext"
...
InstallMosesTools () {
  ...
  working_dir="${tools_ext}/moses-tokenizer/tokenizer"
  MKDIR "${working_dir}"
  ...
  moses_non_breaking_langs=( \
      ...
      "ta" "yue" "zh" )
  working_dir="${tools_ext}/moses-tokenizer/share/nonbreaking_prefixes"
  MKDIR "${working_dir}"
  cd "${working_dir}" || exit 1
...
InstallSentencePiece () {
  ...
  if [ ! -d sentencepiece-master ] ; then
    ...
    if [ ! -s /usr/local/bin/spm_encode ] ; then
      echo " - building code "
      cd sentencepiece-master || exit 1
      mkdir build
      cd build || exit 1
      cmake ..
      make -j 10
...
# We do not use automatic installation with "pip" but directly add the source directory
#
###################################################################
...
InstallMecab () {
  ...
  if [ ! -x mecab/mecab/bin/mecab ] ; then
    ...
    if [ ! -d mecab/lib/mecab/dic/ipadic ] ; then
      cd "${tools_ext}/mecab-master/mecab-ipadic" || exit 1
